...
def star_two(filepath):
    ...
    for j in range(len(lines)):
        for i in range(len(lines[j])):
            ...
            is_downwards = lambda xss, word: xss[j + 1][i + 1] == word[0] and xss[j - 1][i - 1] == word[1]
            is_upwards = lambda xss, word: xss[j + 1][i - 1] == word[0] and xss[j - 1][i + 1] == word[1]
            is_an_x_max = (is_downwards(lines, "MS") or is_downwards(lines, "SM")) and (is_upwards(lines, "MS") or is_upwards(lines, "SM"))
